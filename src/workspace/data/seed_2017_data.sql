...
-- SEED 2017 TAX DATA
-- =====================================================
-- Seeds chart_of_accounts with enriched line items,
-- tax_entries with known amounts, and tax_carryforwards.
-- Run AFTER deploy_tax_persistence.sql
-- =====================================================

BEGIN;

-- =====================================================
-- HELPER: Get current user ID for seeding
-- =====================================================
-- This script uses auth.uid() so it must be run in a context
-- where the user is authenticated. For SQL Editor seeding,
-- replace with a literal UUID if needed.

DO $$
DECLARE
  v_user_id UUID;
  -- Account UUIDs (generated here so we can reference them for entries + children)
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Must be authenticated to seed data. Run from an authenticated context.';
  END IF;

  -- Clean up existing seed data for this user (optional, removes duplicates)
  DELETE FROM finance.tax_entries WHERE user_id = v_user_id AND year = 2017;
  DELETE FROM finance.chart_of_accounts WHERE user_id = v_user_id 
    AND form_line IS NOT NULL; -- Only delete enriched items (preserve legacy COA)
  ...
  VALUES
    ...
    (v_int_div, v_user_id, 'Interest & Dividends', 'INCOME', 'income', 40, 
     'Lines 8a/9a', '2017_Form_1040.pdf', '1099-INT/DIV', false, false),
    
    (v_ira_dist, v_user_id, 'IRA Distributions / Pensions', 'INCOME', 'income', 50, 
     'Lines 15-16', '2017_Form_1040.pdf', '1099-R', false, false);
  ...
  VALUES
    ...
    (v_state_local, v_user_id, 'State & Local Taxes Paid', 'EXPENSE', 'deductions', 30, 
    ...
    (v_medical, v_user_id, 'Medical & Dental Expenses', 'EXPENSE', 'deductions', 50, 
  ...
  -- =====================================================
  -- Net long-term capital loss was -$76,555 (Schedule D)
  -- IRS allows max $3,000/year deduction
  -- Remaining $73,555 carries forward
  ...
  VALUES
    (v_user_id, 'CAPITAL_LOSS', 2017, -76555.00, 2017, -3000.00, -73555.00, 
     'Net LT capital loss from Fidelity account. $3,000 max deduction per IRC §1211(b).');

  RAISE NOTICE '✅ 2017 tax data seeded: 24 COA items, 18 entries, 1 carryforward';
END;
$$;
